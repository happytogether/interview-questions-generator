...
export default function Items(props) {
  ...
  return (
    <li key={index} value={props.value}  className={`item flex items-center justify-center ${flexDirection[index%2]} ${colorPalette[index%4]}`}>
      ...
      >
        ...
        onMouseLeave={() => cursorChangeHandler("")} className="text-left border rounded-sm py-3 px-6" onClick={() => { props.props.handleCategoryChange(item.cat); props.props.getCategoryIndex(item.index);props.openQuestions(); props.lockBodyScrolling();props.handleRandomBg(randomBg);}}>View Questions</button>
        </figcaption>
      </HoverIntent>
