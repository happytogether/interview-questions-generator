...
export default function HomeHeader(props) {
  ...
  return (
    ...
    initial="initial" onMouseMove={({ clientX: x, clientY: y }) => set({ xy: calc(x, y) })} className="h-screen w-screen flex flex-row homehead lg:flex-col lg:h-auto">
      <div onMouseMove={({ clientX: x, clientY: y }) => set({ xy: calc(x, y) })} className={`w-3/5 overflow-hidden relative h-full lg:w-full lg:h-screen lg:py-20`}>
        ...
        <motion.div variants={downMotion}>
          <animated.div className="lg:hidden absolute left-2/3 top-2/3" style={{ transform: props1.xy.interpolate(trans5) }}>
            <Moon2 />
          </animated.div>
        </motion.div>
      </div>
      <div className={`w-2/5 overflow-hidden flex items-center justify-center lg:w-full lg:h-auto lg:py-20 relative`}>
        <motion.div className="bg-gray-light flex items-center w-full h-screen" variants={pageVariants} transition={pageTransition} initial='initial' animate='in' exit='leftOut'>
          <div className="w-9/12 sm:w-11/12 md:w-9/12 mt-10 text-2xl lg:text-xl homehead-right ml-20 lg:ml-0 lg:mt-20">
            <motion.div variants={pageVariants} transition={pageTransition} initial='initial' animate='in' exit='leftOut' className={`text-left p-8`}>
              <span className="p-3 my-3 bg-white leading-loose"><span className="">Anni Wang</span> is an UX Engineer, a Design Technologist. </span><br />
          ...
          <div className="flex flex-col justify-center absolute left-0 top-0 h-full" style={{"width": "50px"}}>
              {

                [0,1,2,3,4].map((item, index) => (
                    <InviewBar2 index={index} />
                ))
